...
use css::selectors::fuzz::{
    SelectorParserFuzzConfig, run_seeded_selector_parser_fuzz_case,
};
...
fuzz_target!(|data: &[u8]| {
    ...
    if let Err(err) = run_seeded_selector_parser_fuzz_case(data, config) {
        panic!("css selector parser fuzz invariant failed: {err}");
    }
});
