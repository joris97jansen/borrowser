...
CORPUS_DIR="${ROOT_DIR}/fuzz/corpus/html5_tree_builder_tokens"
REGRESSION_DIR="${ROOT_DIR}/fuzz/regressions/html5_tree_builder_tokens"
BIN_DIR="${ROOT_DIR}/fuzz/target/debug"
BIN_PATH="${BIN_DIR}/html5_tree_builder_tokens"
ARTIFACT_DIR="${ROOT_DIR}/ci_artifacts"
ARTIFACT_BASENAME="${HTML5_TREE_BUILDER_TOKEN_FUZZ_ARTIFACT_BASENAME:-html5_tree_builder_token_fuzz_failure}"
ARTIFACT_PATH="${ARTIFACT_DIR}/${ARTIFACT_BASENAME}"
LABEL="${HTML5_TREE_BUILDER_TOKEN_FUZZ_LABEL:-html5 tree-builder token fuzz smoke}"

SEED="${HTML5_TREE_BUILDER_TOKEN_FUZZ_SMOKE_SEED:-3141592653}"
RUNS="${HTML5_TREE_BUILDER_TOKEN_FUZZ_SMOKE_RUNS:-128}"
INPUT_TIMEOUT_SEC="${HTML5_TREE_BUILDER_TOKEN_FUZZ_SMOKE_INPUT_TIMEOUT_SEC:-5}"
WALL_TIMEOUT_SEC="${HTML5_TREE_BUILDER_TOKEN_FUZZ_SMOKE_WALL_TIMEOUT_SEC:-90}"
...
cargo build --manifest-path "${ROOT_DIR}/fuzz/Cargo.toml" --bin html5_tree_builder_tokens
...
if [[ "${run_failure}" -ne 0 ]]; then
  if [[ -f "${ARTIFACT_PATH}" ]]; then
    ...
    CARGO_FUZZ_REPRO_CMD=("cargo" "fuzz" "run" "html5_tree_builder_tokens" "${ARTIFACT_PATH}")
  else
    DIRECT_REPRO_CMD=("${SMOKE_CMD[@]}")
    CARGO_FUZZ_REPRO_CMD=("cargo" "fuzz" "run" "html5_tree_builder_tokens" "${INPUT_DIRS[@]}")
  fi
  echo
  echo "${LABEL} failed"
  echo "  seed: ${SEED}"
