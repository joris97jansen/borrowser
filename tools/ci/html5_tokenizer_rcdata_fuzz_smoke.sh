...
CORPUS_DIR="${ROOT_DIR}/fuzz/corpus/html5_tokenizer_rcdata"
REGRESSION_DIR="${ROOT_DIR}/fuzz/regressions/html5_tokenizer_rcdata"
BIN_DIR="${ROOT_DIR}/fuzz/target/debug"
BIN_PATH="${BIN_DIR}/html5_tokenizer_rcdata"
ARTIFACT_DIR="${ROOT_DIR}/ci_artifacts"
ARTIFACT_BASENAME="${HTML5_TOKENIZER_RCDATA_FUZZ_ARTIFACT_BASENAME:-html5_tokenizer_rcdata_fuzz_failure}"
ARTIFACT_PATH="${ARTIFACT_DIR}/${ARTIFACT_BASENAME}"
LABEL="${HTML5_TOKENIZER_RCDATA_FUZZ_LABEL:-html5 tokenizer rcdata fuzz smoke}"

SEED="${HTML5_TOKENIZER_RCDATA_FUZZ_SMOKE_SEED:-2246822519}"
RUNS="${HTML5_TOKENIZER_RCDATA_FUZZ_SMOKE_RUNS:-128}"
INPUT_TIMEOUT_SEC="${HTML5_TOKENIZER_RCDATA_FUZZ_SMOKE_INPUT_TIMEOUT_SEC:-5}"
WALL_TIMEOUT_SEC="${HTML5_TOKENIZER_RCDATA_FUZZ_SMOKE_WALL_TIMEOUT_SEC:-90}"
...
cargo build --manifest-path "${ROOT_DIR}/fuzz/Cargo.toml" --bin html5_tokenizer_rcdata
...
if [[ "${run_failure}" -ne 0 ]]; then
  if [[ -f "${ARTIFACT_PATH}" ]]; then
    ...
    CARGO_FUZZ_REPRO_CMD=("cargo" "fuzz" "run" "html5_tokenizer_rcdata" "${ARTIFACT_PATH}")
  else
    DIRECT_REPRO_CMD=("${SMOKE_CMD[@]}")
    CARGO_FUZZ_REPRO_CMD=("cargo" "fuzz" "run" "html5_tokenizer_rcdata" "${INPUT_DIRS[@]}")
  fi
  echo
  echo "${LABEL} failed"
  echo "  seed: ${SEED}"
