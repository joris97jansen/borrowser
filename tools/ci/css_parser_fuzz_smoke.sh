...
CORPUS_DIR="${ROOT_DIR}/fuzz/corpus/css_parser"
REGRESSION_DIR="${ROOT_DIR}/fuzz/regressions/css_parser"
BIN_DIR="${ROOT_DIR}/fuzz/target/debug"
BIN_PATH="${BIN_DIR}/css_parser"
ARTIFACT_DIR="${ROOT_DIR}/ci_artifacts"
ARTIFACT_BASENAME="${CSS_PARSER_FUZZ_ARTIFACT_BASENAME:-css_parser_fuzz_failure}"
ARTIFACT_PATH="${ARTIFACT_DIR}/${ARTIFACT_BASENAME}"
LABEL="${CSS_PARSER_FUZZ_LABEL:-css parser fuzz smoke}"

SEED="${CSS_PARSER_FUZZ_SMOKE_SEED:-2718281828}"
RUNS="${CSS_PARSER_FUZZ_SMOKE_RUNS:-128}"
INPUT_TIMEOUT_SEC="${CSS_PARSER_FUZZ_SMOKE_INPUT_TIMEOUT_SEC:-5}"
WALL_TIMEOUT_SEC="${CSS_PARSER_FUZZ_SMOKE_WALL_TIMEOUT_SEC:-90}"
...
cargo build --manifest-path "${ROOT_DIR}/fuzz/Cargo.toml" --locked --bin css_parser
...
if [[ "${run_failure}" -ne 0 ]]; then
  if [[ -f "${ARTIFACT_PATH}" ]]; then
    ...
    CARGO_FUZZ_REPRO_CMD=("cargo" "fuzz" "run" "css_parser" "${ARTIFACT_PATH}")
  else
    DIRECT_REPRO_CMD=("${SMOKE_CMD[@]}")
    CARGO_FUZZ_REPRO_CMD=("cargo" "fuzz" "run" "css_parser" "${INPUT_DIRS[@]}")
  fi
  echo
  echo "${LABEL} failed"
  echo "  seed: ${SEED}"
