...
| `TB-MODE-IN-BODY` | MVP | `In body` | `#parsing-main-inbody` | `dispatch/in_body.rs`, `dispatch/select.rs`, `body_recovery.rs`, `html_semantics.rs`, `stack/end_tag.rs` | Current: `tree_builder/text`, `tree_builder/simple-element`, `ae9b-*`; WPT: core cases plus the fixed AE9b select subset. | Character/comment/generic insertion, supported formatting hooks, current select-family rules, and one-pass generic end-tag special barriers. | `tb-in-body-core`, `tb-ae9b-select` | Main workload mode; AE9b adds current select behavior without a select-specific mode. |
