...
use html::{ElementNamespace, Node};

/// Layout-owned decision made before display-driven box classification.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum BoxGenerationDecision {
    Generate,
...
pub enum BoxSuppressionReason {
    ...
    DisplayNone,
    UnsupportedForeignNamespace(ElementNamespace),
}

pub(super) fn namespace_box_generation_decision(styled: &StyledNode<'_>) -> BoxGenerationDecision {
    match styled.node {
        Node::Element { element } if element.namespace() != ElementNamespace::Html => {
            BoxGenerationDecision::SuppressSubtree(
                BoxSuppressionReason::UnsupportedForeignNamespace(element.namespace()),
            )
        }
        _ => BoxGenerationDecision::Generate,
    }
}
