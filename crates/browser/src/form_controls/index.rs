...
impl<S: InputStore> FormControlHandler<S> for FormControlIndex {
    fn on_radio_clicked(&self, store: &mut S, radio_id: InputId) -> bool {
        // Convert InputId to html::internal::Id for group lookup, then use InputId for store operations
...
impl RadioGroupIndex {
    ...
    ///
    /// Converts `InputId` to `html::internal::Id` for group lookup, then uses the
