...
/// Total current source order for stylesheet and inline declarations.
///
/// Inline-style precedence is still primarily supplied by inline specificity;
/// this cross-kind ordering exists to keep the cascade key lawful and total.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub enum CascadeSourceOrder {
    Stylesheet(StylesheetRuleOrder),
    InlineStyle,
}

impl CascadeSourceOrder {
    pub fn semantic_cmp(self, other: Self) -> std::cmp::Ordering {
        match (self, other) {
            (Self::Stylesheet(left), Self::Stylesheet(right)) => left.semantic_cmp(right),
            (Self::Stylesheet(_), Self::InlineStyle) => std::cmp::Ordering::Less,
            (Self::InlineStyle, Self::Stylesheet(_)) => std::cmp::Ordering::Greater,
            (Self::InlineStyle, Self::InlineStyle) => std::cmp::Ordering::Equal,
        }
    }
}

impl Ord for CascadeSourceOrder {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        self.semantic_cmp(*other)
    }
}

impl PartialOrd for CascadeSourceOrder {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        Some(self.cmp(other))
    }
}
...
#[cfg(test)]
impl From<u32> for CascadeSourceOrder {
    fn from(style_rule_position: u32) -> Self {
        Self::Stylesheet(style_rule_position.into())
    }
}
