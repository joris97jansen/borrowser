...
def main() -> None:
    ...
    semi_table: list[tuple[bytes, str, str]] = []
    legacy_table: list[tuple[bytes, str, str]] = []
    max_len = 0
    max_legacy_len = 0
    ...
    for name, payload in entities.items():
        ...
        if name.endswith(";"):
            ...
            semi_table.append((name_bytes, name, chars))
        else:
            max_legacy_len = max(max_legacy_len, len(name_bytes))
            legacy_table.append((name_bytes, name, chars))

    semi_table.sort(key=lambda item: item[0])
    legacy_table.sort(key=lambda item: item[0])
    ...
    lines = [
        ...
        "}",
        "",
        f"pub(crate) const MAX_HTML5_ENTITY_LEN: usize = {max(max_len, max_legacy_len)};",
        f"pub(crate) const MAX_HTML5_LEGACY_ENTITY_LEN: usize = {max_legacy_len};",
        "",
        "#[rustfmt::skip]",
        "pub(crate) static HTML5_ENTITIES: &[Html5Entity] = &[",
    ]

    for _, name, chars in semi_table:
    ...
    lines.append("pub(crate) static HTML5_LEGACY_ENTITIES: &[Html5Entity] = &[")

    for _, name, chars in legacy_table:
        name_literal = rust_byte_string_literal(name)
        value_literal = rust_string_literal(chars)
        lines.append(f"    Html5Entity {{ name: {name_literal}, value: {value_literal} }},")
