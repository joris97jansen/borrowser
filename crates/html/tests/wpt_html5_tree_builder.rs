...
fn wpt_html5_tree_builder_slice() {
    ...
    let cases = select_tree_builder_cases(cases, &run_config, &skip_overrides, &manifest_path);
    assert!(
        !cases.is_empty(),
        "no tree-builder WPT cases selected after filters and skips"
    );
    ...
    if !failures.is_empty() {
        ...
        let _ = writeln!(
            &mut report,
            "WPT tree-builder run summary: total={} passed={} failed={} xfailed={} xpass={} skipped={}",
...
fn select_tree_builder_cases(
    ...
    if has_filters && selected.is_empty() {
        panic!(
            "no tree-builder WPT cases matched filters in {manifest_path:?} (WPT_FILTER={:?}, WPT_IDS={:?})",
