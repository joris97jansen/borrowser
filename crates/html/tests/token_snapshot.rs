        ...
        let line = match token {
            ...
            Token::Comment { text } => {
                let text = resolver.resolve_span(*text).ok_or_else(|| {
                    format!(
                        "invalid text span in '{}' [{}] token #{} (comment)",
                        context.case_id, context.mode, token_index
                    )
                })?;
            ...
            Token::Character { span } => {
                let text = resolver.resolve_span(*span).ok_or_else(|| {
                    format!(
                        "invalid text span in '{}' [{}] token #{} (char)",
                        context.case_id, context.mode, token_index
                    )
                })?;
...
fn format_attr(
    ...
    match &attr.value {
        ...
        Some(AttributeValue::Span(span)) => {
            let value = resolver.resolve_span(*span).ok_or_else(|| {
                format!(
                    "invalid attribute value span in '{}' [{}] token #{} attr #{} ({})",
                    context.case_id, context.mode, token_index, attr_index, name
                )
            })?;
