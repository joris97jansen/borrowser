...
use html_test_support::wpt_tokenizer::{parse_env_bool, parse_u64};
use html_test_support::wpt_tree_builder::{
    TreeBuilderSkipOverride, TreeBuilderSkipStatus, applied_skip_override, ensure_utf8_plan,
    load_tree_builder_skip_overrides, run_tree_builder_chunked, run_tree_builder_whole,
...
#[allow(dead_code)]
impl TreeBuilderSuiteSpec {
    pub(crate) const fn all() -> Self {
        Self {
            suite_name: "WPT tree-builder",
            required_ids: &[],
        }
    }

    pub(crate) const fn formatting() -> Self {
        Self {
            suite_name: "WPT tree-builder formatting/adoption-agency",
            required_ids: &[
                "formatting-nested-inline-attrs",
                "formatting-aaa-misnest-b-i",
                "formatting-aaa-misnest-a-b-attrs",
                "formatting-reconstruct-single",
                "formatting-reconstruct-multi",
                "formatting-special-anchor-recovery",
                "formatting-special-nobr-recovery",
                "formatting-marker-applet-isolation",
            ],
        }
    }
}
...
pub(crate) fn run(spec: TreeBuilderSuiteSpec) {
    ...
    let skip_overrides = load_tree_builder_skip_overrides(&wpt_root());
    let dom_case_ids = cases
        .iter()
        .filter(|case| case.kind == CaseKind::Dom)
        .map(|case| case.id.clone())
        .collect::<BTreeSet<_>>();
    validate_skip_override_ids(&skip_overrides, &dom_case_ids, &manifest_path);
    validate_suite_ids(spec, &dom_case_ids, &manifest_path);
    let cases =
        select_tree_builder_cases(cases, spec, &run_config, &skip_overrides, &manifest_path);
    assert!(
        !cases.is_empty(),
        "no {} cases selected after filters and skips",
        spec.suite_name
    );
    ...
    for case in cases {
        ...
        // golden fixture loaders, we do not strip a terminal formatting
        // newline, because file bytes are part of the externally-facing
        // conformance contract for this suite.
        ...
        match run_tree_builder_whole(&input, &case.id, options) {
            ...
            Err(err) => {
                failure = Some(format!(
                    "WPT tree-builder case '{}' failed ({}) error: {err}",
                    case.id,
                    case.path.display()
                ));
            }
        ...
        if run_config.chunked && (failure.is_none() || run_config.chunked_force) {
            if chunk_plans.is_empty() {
                failure = Some(format!(
                    "WPT tree-builder case '{}' requested chunked mode but no chunk plans were generated",
                    case.id
                ));
            } else {
                for plan in &chunk_plans {
                    ...
                    match run_tree_builder_chunked(
                        ...
                        Ok(lines) => {
                            let (diff_basis, diff_target): (&str, &[String]) =
                                match whole_lines.as_ref() {
                                    ...
                                    None => ("expected-vs-chunked", expected.lines.as_slice()),
                                };
                        ...
                        Err(err) => {
                            ...
                            failure = Some(format!(
                                "WPT tree-builder case '{}' failed ({}) [chunked: {}] error: {err}\nshrunk: {}\nshrink stats: {:?}\ninput file: {:?}",
                                case.id,
                                case.path.display(),
                                plan.label,
                                shrunk,
            ...
            FixtureStatus::Xfail => match failure {
                ...
                None => {
                    ...
                    failures.push(Failure {
                        id: case.id.clone(),
                        message: format!(
                            "WPT tree-builder case '{}' matched expected DOM but is marked xfail; reason: {}",
                            case.id,
    ...
    if !failures.is_empty() {
        ...
        let _ = writeln!(
            &mut report,
            "{} run summary: total={} passed={} failed={} xfailed={} xpass={} skipped={}",
            spec.suite_name, total, passed, failed, xfailed, xpass, skipped
        );
...
fn validate_suite_ids(
    spec: TreeBuilderSuiteSpec,
    ...
    for id in spec.required_ids {
        assert!(
            ...
            "{} references missing DOM case id '{}' in {:?}",
            spec.suite_name,
            id,
            manifest_path
        );
...
fn select_tree_builder_cases(
    ...
    let suite_ids = spec
        ...
        .map(str::to_string)
        .collect::<BTreeSet<_>>();
    let suite_is_fixed = !suite_ids.is_empty();
    let mut selected = Vec::new();
    for mut case in cases {
        if case.kind != CaseKind::Dom {
            continue;
        }
        if suite_is_fixed && !suite_ids.contains(&case.id) {
            continue;
        }
    ...
    if has_filters && selected.is_empty() {
        panic!(
            "no {} cases matched filters in {manifest_path:?} (WPT_FILTER={:?}, WPT_IDS={:?})",
            spec.suite_name, run_config.filter, run_config.ids
        );
    }
