...
pub(crate) struct TokenizerSuiteSpec {
    suite_name: &'static str,
    required_ids: &'static [&'static str],
}

#[allow(dead_code)]
impl TokenizerSuiteSpec {
    pub(crate) const fn all() -> Self {
        Self {
            suite_name: "WPT tokenizer",
            required_ids: &[],
        }
    }

    pub(crate) const fn rawtext_script() -> Self {
        Self {
            suite_name: "WPT rawtext/RCDATA/script tokenizer",
            required_ids: &[
                "tokenizer-rawtext-style",
                "tokenizer-rcdata-title-entities",
...
pub(crate) fn run(spec: TokenizerSuiteSpec) {
    ...
    validate_suite_ids(spec, &token_case_ids, &manifest_path);
    let cases = select_tokenizer_cases(cases, spec, &run_config, &skip_overrides, &manifest_path);
    assert!(
        !cases.is_empty(),
    ...
    for case in cases {
        ...
        if run_config.chunked && (failure.is_none() || run_config.chunked_force) {
            if chunk_plans.is_empty() {
                failure = Some(format!(
                    "{} case '{}' requested chunked mode but no chunk plans were generated",
                    spec.suite_name, case.id
                ));
            } else {
            ...
            FixtureStatus::Xfail => match failure {
                ...
                None => {
                    ...
                    failures.push(Failure {
                        id: case.id.clone(),
                        message: format!(
                            "{} case '{}' matched expected tokens but is marked xfail; reason: {}",
                            spec.suite_name,
                            case.id,
...
fn validate_suite_ids(
    ...
    for id in spec.required_ids {
        assert!(
            ...
            "{} references missing token case id '{}' in {:?}",
            spec.suite_name,
            id,
            manifest_path
        );
