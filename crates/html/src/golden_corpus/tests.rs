use super::{AllowedFailure, FixtureKind, GoldenFixture, Invariant, fixtures};
...
fn validate_allowed(expectation: super::Expectation, invariants: &[Invariant], name: &str) {
...
fn run_golden_fixture(fixture: &GoldenFixture, plans: &[ChunkPlan]) -> FixtureRun {
    ...
    for plan in plans {
        ...
        for &inv in fixture.invariants {
            let result = check_invariant(fixture, inv, &full_dom, &chunked_dom, &chunked_tokens);
            match result {
                Ok(()) => {
                    if let Some(reason) = is_allowed_to_fail(fixture, inv) {
                        if strict_xpass {
...
fn is_allowed_to_fail(fixture: &GoldenFixture, invariant: Invariant) -> Option<&'static str> {
