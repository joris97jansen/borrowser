...
fn session_reconstruction_is_chunk_equivalent_after_generic_ancestor_pop() {
    fn run_session(chunks: &[&str]) -> Vec<DomPatch> {
        let ctx = DocumentParseContext::new();
        let mut session = Html5ParseSession::new(
            TokenizerConfig::default(),
            TreeBuilderConfig::default(),
        ...
        for chunk in chunks {
            ...
            session
                .pump()
                .expect("reconstruction scenario chunk should remain recoverable");
        }

        session
            .finish_for_test()
            .expect("reconstruction scenario should finish cleanly");
        session.take_patches()
    }

    let whole = run_session(&["<!doctype html><div><b class=\"x\">one</div>two"]);
    let chunked = run_session(&[
...
fn session_reconstruction_of_multiple_missing_formatting_elements_is_chunk_equivalent() {
    fn run_session(chunks: &[&str]) -> Vec<DomPatch> {
        ...
        for chunk in chunks {
            ...
            session
                .pump()
                .expect("multi-element reconstruction chunk should remain recoverable");
        }

        session
            .finish_for_test()
            .expect("multi-element reconstruction scenario should finish cleanly");
        session.take_patches()
    }

    let whole = run_session(&["<!doctype html><div><b class=\"x\"><i class=\"y\">one</div>two"]);
    let chunked = run_session(&[
        "<!doctype html><div><b class=\"x\">",
        "<i class=\"y\">one",
        "</div>t",
